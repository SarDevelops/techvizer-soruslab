$(document).ready(function () {
	if ($('#kt_table_cut').length) {
		var cut_table = $('#kt_table_cut').DataTable({
			info: true,
			pageLength: 10,
			responsive: true,
			processing: true,
	...
	// Show add edit cut form
	$(document).on('click', '.open-cut-form', function () {
		show_loader();
		let method = 'GET';
	...
	$(document).on('change', '#cut_form', function () {
		$('#cut_form').valid();
	});

	// Add & Edit the cut detail
	$(document).on('submit', '#cut_form', function (event) {
		event.preventDefault();

		show_loader();
		$('.submit-btn').prop('disabled', true);
		let cut_form = $(this);
		let method = cut_form.attr('method');
		let url = cut_form.attr('action');
		let data = new FormData($(cut_form)[0]);
	...
	// Delete the cut detail
	$(document).on('click', '.delete-cut', function (event) {
		event.preventDefault();
		let delete_element = $(this);

		delete_confirmation('Are you sure you want to delete this cut?').then(function (response) {
			if (response['isConfirmed']) {
				delete_cuts(delete_element);
			}
		});
	});

	$(document).on('keyup', '[data-kt-cut-table-filter="search"]', delay(function (event) {
		cut_table.search(event.target.value).draw();
	}, 500));
	...
	$(document).on('change', 'td:first-child input[type="checkbox"], th:first-child input[type="checkbox"]', function () {
		...
		if (checkboxes_count > 0) {
			...
			$('[data-kt-cut-table-select="selected_count"]').text(checkboxes_count);
		} else {
	...
	$(document).on('click', '[data-kt-cut-table-select="delete_selected"]', function () {
		...
		if (selected_ids.length) {
			...
			delete_confirmation('Are you sure you want to delete this cut?').then(function (response) {
				if (response['isConfirmed']) {
					delete_cuts(delete_element, selected_ids);
				}
			});
		}
	});
	...
	$(document).on('change', '.is_active', function () {
		...
		delete_confirmation(`Are you sure you want to ${status_name} the cut?`, `Yes, ${status_name} it!`, 'No, Revert it!').then(function (response) {
			if (response['isConfirmed']) {
	...
	function delete_cuts(element, selected_ids = []) {
		let method = 'DELETE';
		let url = element.attr('target-url');
