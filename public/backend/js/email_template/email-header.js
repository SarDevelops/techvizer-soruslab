...
$(document).ready(function () {
	...
	$('.copy').on('click', function (e) {
		...
		CKEDITOR.instances['header_body'].insertText(placeholder_value);
	});

	let $edit_email_header_form = $("#edit_email_header_form").validate({
	    errorElement: "div",
	    ...
	    submitHandler: function (e) {
	        $(e).ajaxSubmit({
	            url: $('#edit_email_header_form').attr('action'),
	            dataType:'json',
	            type: 'POST',
	            clearForm: true,
	            beforeSubmit: function (formData, jqForm, options) {
	                $('#btn_edit_email_header').attr('data-kt-indicator', 'on');
	                $('#btn_edit_email_header').attr('disabled', 'disabled');
	            },
	            complete: function () {
	                $('#btn_edit_email_header').removeAttr('data-kt-indicator');
	                $('#btn_edit_email_header').removeAttr('disabled');
	            },
	            error: function ( error ) {
					let $error = error.responseJSON.errors;
	                $edit_email_header_form.showErrors($error);
	            },
	            success: function (data) {
	                if (data.status == true) {
