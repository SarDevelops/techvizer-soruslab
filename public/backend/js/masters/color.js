$(document).ready(function () {
	if ($('#kt_table_color').length) {
		var color_table = $('#kt_table_color').DataTable({
			info: true,
			pageLength: 10,
			responsive: true,
			processing: true,
	...
	// Show add edit color form
	$(document).on('click', '.open-color-form', function () {
		show_loader();
		let method = 'GET';
	...
	$(document).on('change', '#color_form', function () {
		$('#color_form').valid();
	});
	// Add & Edit the color detail
	$(document).on('submit', '#color_form', function (event) {
		...
		let color_form = $(this);
		let method = color_form.attr('method');
		let url = color_form.attr('action');
		let data = new FormData($(color_form)[0]);
	...
	// Delete the color detail
	$(document).on('click', '.delete-color', function (event) {
		event.preventDefault();
		let delete_element = $(this);

		delete_confirmation('Are you sure you want to delete this color?').then(function (response) {
			if (response['isConfirmed']) {
				delete_colors(delete_element);
			}
		});
	});

	$(document).on('keyup', '[data-kt-color-table-filter="search"]', delay(function (event) {
		color_table.search(event.target.value).draw();
	}, 500));
	...
	$(document).on('change', 'td:first-child input[type="checkbox"], th:first-child input[type="checkbox"]', function () {
		...
		if (checkboxes_count > 0) {
			...
			$('[data-kt-color-table-select="selected_count"]').text(checkboxes_count);
		} else {
	...
	$(document).on('click', '[data-kt-color-table-select="delete_selected"]', function () {
		...
		if (selected_ids.length) {
			...
			delete_confirmation('Are you sure you want to delete this color?').then(function (response) {
				if (response['isConfirmed']) {
					delete_colors(delete_element, selected_ids);
				}
			});
		}
	});
	...
	$(document).on('change', '.is_active', function () {
		...
		delete_confirmation(`Are you sure you want to ${status_name} the color?`, `Yes, ${status_name} it!`, 'No, Revert it!').then(function (response) {
			if (response['isConfirmed']) {
	...
	function delete_colors(element, selected_ids = []) {
		let method = 'DELETE';
		let url = element.attr('target-url');
