$(document).ready(function () {
	if ($("#kt_table_shape").length) {
		var shape_table = $("#kt_table_shape").DataTable({
			info: true,
			responsive: true,
			processing: true,
			serverSide: true,
	...
	$(document).on("click", ".open-shape-form", function () {
		show_loader();
		let method = "GET";
	...
	$(document).on("change", "#shape_form", function () {
		$("#shape_form").valid();
	});

	// Add & Edit the shape detail
	$(document).on("submit", "#shape_form", function (event) {
		event.preventDefault();

		show_loader();
		$(".submit-btn").prop("disabled", true);
		let shape_form = $(this);
		let method = shape_form.attr("method");
		let url = shape_form.attr("action");
		let data = new FormData($(shape_form)[0]);
	...
	// Delete the shape detail
	$(document).on("click", ".delete-shape", function (event) {
		event.preventDefault();
		let delete_element = $(this);

		delete_confirmation("Are you sure you want to delete this shape?").then(
			function (response) {
				if (response["isConfirmed"]) {
					delete_shapes(delete_element);
				}
			}
		);
	});
	...
	$(document).on(
		"keyup",
		'[data-kt-shape-table-filter="search"]',
		delay(function (event) {
			shape_table.search(event.target.value).draw();
		}, 500)
	);
	...
	$(document).on(
		...
		function () {
			...
			if (checkboxes_count > 0) {
				...
				$('[data-kt-shape-table-select="selected_count"]').text(
					checkboxes_count
				);
			} else {
	...
	$(document).on(
		"click",
		'[data-kt-shape-table-select="delete_selected"]',
		function () {
			...
			if (selected_ids.length) {
				...
				delete_confirmation(
					...
					if (response["isConfirmed"]) {
						delete_shapes(delete_element, selected_ids);
					}
				});
			}
		}
	...
	$(document).on("change", ".is_active", function () {
		...
		delete_confirmation(
			`Are you sure you want to ${status_name} the shape?`,
			`Yes, ${status_name} it!`,
			"No, Revert it!"
		).then(function (response) {
			if (response["isConfirmed"]) {
	...
	function delete_shapes(element, selected_ids = []) {
		let method = "DELETE";
		let url = element.attr("target-url");
