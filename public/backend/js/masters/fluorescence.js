$(document).ready(function () {
	if ($('#kt_table_fluorescence').length) {
		var fluorescence_table = $('#kt_table_fluorescence').DataTable({
			info: true,
            responsive: true,
            processing: true,
            serverSide: true,
			...
			ajax: {
				url: fluorescence_listing_route,
				data: function (data) {
	...
	$(document).on('click', '.open-fluorescence-form', function () {
		show_loader();
		let method = 'GET';
	...
	$(document).on('change', '#fluorescence_form', function () {
		$('#fluorescence_form').valid();
	});

	// Add & Edit the fluorescence detail
	$(document).on('submit', '#fluorescence_form', function (event) {
		event.preventDefault();

		show_loader();
		$('.submit-btn').prop('disabled', true);
		let fluorescence_form = $(this);
		let method = fluorescence_form.attr('method');
		let url = fluorescence_form.attr('action');
		let data = new FormData($(fluorescence_form)[0]);
		...
		ajax.done(function (response) {
			...
			if (response['FLAG']) {
                ...
                action_drawer();
				setTimeout(() => {
					fluorescence_table.ajax.reload();
				}, 1000);
	...
	// Delete the fluorescence detail
	$(document).on('click', '.delete-fluorescence-type', function (event) {
		event.preventDefault();
		let delete_element = $(this);

		delete_confirmation('Are you sure you want to delete this fluorescence?').then(function (response) {
			if (response['isConfirmed']) {
				delete_fluorescence(delete_element);
			}
		});
	});

	$(document).on('keyup', '[data-kt-fluorescence-table-filter="search"]', delay(function (event) {
		fluorescence_table.search(event.target.value).draw();
	}, 500));
	...
	$(document).on('change', 'td:first-child input[type="checkbox"], th:first-child input[type="checkbox"]', function () {
		...
		if (checkboxes_count > 0) {
			...
			$('[data-kt-fluorescence-table-select="selected_count"]').text(checkboxes_count);
		} else {
	...
	$(document).on('click', '[data-kt-fluorescence-table-select="delete_selected"]', function () {
		...
		if (selected_ids.length) {
			...
			delete_confirmation('Are you sure you want to delete this fluorescence?').then(function (response) {
				if (response['isConfirmed']) {
					delete_fluorescence(delete_element, selected_ids);
				}
			});
		}
	});
	...
	$(document).on('change', '.is_active', function () {
		...
		delete_confirmation(`Are you sure you want to ${status_name} the fluorescence?`, `Yes, ${status_name} it!`, 'No, Revert it!').then(function (response) {
			if (response['isConfirmed']) {
	...
	function delete_fluorescence(element, selected_ids = []) {
		let method = 'DELETE';
		let url = element.attr('target-url');
