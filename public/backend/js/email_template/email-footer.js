...
$(document).ready(function () {
	...
	$('.copy').on('click', function (e) {
		...
		CKEDITOR.instances['footer_body'].insertText(placeholder_value);
	});

	let $edit_email_footer_form = $("#email_footer_form").validate({
	    ...
	    submitHandler: function (e) {
	        $(e).ajaxSubmit({
	            url: $('#edit_email_footer_form').attr('action'),
	            dataType:'json',
	            type: 'POST',
	            clearForm: false,
	            beforeSubmit: function (formData, jqForm, options) {
	                $('#btn_edit_email_footer').attr('data-kt-indicator', 'on');
	                $('#btn_edit_email_footer').attr('disabled', 'disabled');
	            },
	            complete: function () {
	                $('#btn_edit_email_footer').removeAttr('data-kt-indicator');
	                $('#btn_edit_email_footer').removeAttr('disabled');
	            },
	            error: function ( error ) {
	                let $error = error.responseJSON.errors;
	                $edit_email_footer_form.showErrors($error);
	            },

	            success: function (data) {
	                if (data.status == true) {
	                    toast_message('success',data.msg,'',0,'',1,data.redirect_url);
	                } else {
